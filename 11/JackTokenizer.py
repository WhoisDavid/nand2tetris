...
class JackTokenizer:
    ...
    def xml_converter(self):
        if isinstance(self.current_token, Enum):
            token = self.current_token.value
        else:
            token = self.current_token
        xml_token = re.sub('|'.join(self.xml_chars), lambda x: self.xml_chars[x.group(0)], token)
    ...
    def advance(self):
        ...
        if keyword_or_identifier:
            if keyword_or_identifier.upper() in Keyword.__members__:
                self.current_token, self.token_type = Keyword[keyword_or_identifier.upper()], TokenType.KEYWORD
                return True
            else:
